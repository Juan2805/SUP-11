...
const TeamAdminister = (state = initialState, action) => {
    switch(action.type){
        case 'ADD_SUP': 
            return({...state,
                sup: state.sup.concat(action.payload),
                participants: state.participants.filter( remove => remove.id !== action.payload.id)
            });
        case 'REMOVE_SUP':
            return({...state,
                sup: state.sup.filter( remove => remove.id !== action.payload.id),
                participants: state.participants.concat(action.payload)
            });
        case 'ADD_PENITENCE':
            return({...state,
                penitence: state.penitence.concat(action.payload),
                participants: state.participants.filter( remove => remove.id !== action.payload.id)
            });
        case 'REMOVE_PENITENCE':
            return({...state,
                penitence: state.penitence.filter( remove => remove.id !== action.payload.id),
                participants: state.participants.concat(action.payload)
            });
        default: return ({state})
    }
}
export default createStore(TeamAdminister)